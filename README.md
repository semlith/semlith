...
Every client's configuration stanza lives in [docs/clients.md](docs/clients.md),
which is also what `tests/clients.rs` executes and what the portal's Agents page
shows. Twenty-seven clients are covered, in three groups — terminal, editor and
desktop — and the same file documents the HTTP transport for clients that would
rather hold a URL than spawn a process.
