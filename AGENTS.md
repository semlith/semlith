...
- **A schedule is the daemon's work, and the file is the whole of the state.**
  ...
  their schedules and not every store on the machine. The cadence is stored as
  an interval in seconds: the portal's three chips are shortcuts over a number,
  so a cadence no chip can spell still round-trips. `next_run` is on disk, which
  is the whole of why a restart loses nothing — the runner owes nothing to how
  long the process lived. Two schedules never generate at once
  (`schedule::generating` is one process-wide slot, and any other generator must
  take it too), a due schedule over a store that is still indexing generates
  nothing rather than reporting on a half-written index, and a destination that
  has gone away is recorded in `last_error` and shown. `create_dir_all` is
  deliberately never called on a destination: it would answer an unmounted
  volume by writing into the empty mount point, which looks like success and
  loses the file. The runner sleeps on a condition variable to the next due
  time, capped at a minute — not to poll, but because `semlith schedule add` is
  a different process and cannot reach that condvar.
- **PDF is a renderer, not a second report.** `Report::render` returns a
  `String` and serves the four text formats; `Report::render_bytes` serves all
  five over the same blocks. That is why `report::FORMATS` is four and
  `report::ALL_FORMATS` is five, and why a surface that can only print text
  calls `check_format` while one that can hand back bytes calls
  `check_any_format`. A PDF is typeset from the blocks in base-14 Courier, where
  every glyph is 0.6 em — the width of a line is its length, which is what makes
  the table arithmetic provable and why no font is embedded. It is not a print
  of the HTML.
...
`tests/retrieval.rs` has a fifth question class from 0.27.0, `image`, over a
**second** pinned corpus at `tests/fixtures/retrieval/images/`. It is separate
from the corpus above on purpose: that one's `files` and `bytes` are asserted on
every run because every published figure is a reading of it, so adding pictures
to it would rebaseline hit@k, bytes per answer and the census in one commit. The
class is opted into with `SEMLITH_RETRIEVAL_IMAGES`, so CLIP's download stays
out of a run that is measuring text, and `the_image_class_is_measured` runs it
alone in about a minute. Its corpus is generated by
`tests/fixtures/retrieval/images/generate.py` — regenerate rather than edit, and
re-pin `corpus.yaml` after. The three text files beside the eight pictures are
